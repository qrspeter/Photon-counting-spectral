...
%g В зависимости от того, какой формат короче, применяется либо %e, либо %f
 ...
 Целое число, указанное между знаком процента % и кодом формата, действует как спецификатор минимальной ширины поля.
 Если пус­тующее поле вывода необходимо заполнить нулями, перед спецификатором ширины поля следует поставить символ 0.
...
чтобы добавить модификатор точности, после спецификатора ширины поля необходимо поставить десятичную точку и указать точность.
...
d,i Вывод целого числа со знаком в десятичной систем счисления.
По умолчанию выводится число размером sizeof( int ), с правым выравниванием, указанием знака только для отрицательных чисел.
отрицательный знак нужен т.к. предполагается вычитание фона.

а м.б. нужен и дробный, тк выводится планируется и усреднение. с другой стороны - это на рисование не влияет, это только надписи.
там это лишнее м.б., по крайней менее для числе более 10-100.

сейчас по вертикали предел точности 0.01, переход на экспоненциальную форму не происходит. наверное можно не трогать,
но нули после точки могут мешать при больших сигналах

переход происходит когда диапазон достигает 10 тысяч. во всех случаях видно два знака после запятой.
*/

// endsale - тестирует верхнюю границу, а стоит тестировать диапазон
//	хотя это динамический диапазон... double endscale = (maxScaleAbs > minScaleAbs) ? maxScaleAbs : minScaleAbs;
// но почему-то не работает
		if ((endscale < 1e4) && (endscale > 1e1)) // > 1e-3
				fmt = wxT("%.0f"); // ("%.2f") // подстановка %d крашит преобразование
			else
				fmt = wxT("%.0e"); // .2e - было, но так ненужные сотые когда более 10 тысяч.
		}
		else
		{
			fmt = m_labelFormat;
