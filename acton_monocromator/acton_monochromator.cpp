...
//#include <fstream.h> // Для записи буфера ввода в файл (для отладки).
//#include <string.h> // strlen - подсчет длины строки. http://www.c-cpp.ru/content/strlen-fstrlen
...
// или заменить на строковые? ноль при выводе заменять на команду перевода строки
// очистка буфера перед каждой командой.

// добавить как-то команду переноса строки в конце, можно ведь?
...
/*
static	char ARC_SET_WAVELENGTH[] =     {0x20,'G','O','T','O',0x0D}; // команда перехода на определенную длину волны.
static	char ARC_SET_GRATING[] =        {0x20,'G','R','A','T','I','N','G',0x0D}; //установка определенной решетки.
static  char ACR_GET_GRATING[] =        {'?','G','R','A','T','I','N','G',0x0D};
static  char ARC_EXIT_MIRROR[] =        {'E','X','I','T','-','M','I','R','R','O','R',0x0D};
static	char ARC_SET_MIRROR_FRONT[]  =  {'F','R','O','N','T',0x0D};
static	char ARC_SET_MIRROR_SIDE[] =    {'S','I','D','E',0x0D};
static  char ACR_GET_MIRROR[] =         {'?','M','I','R','R','O','R',0x0D};
*/
...
// // Длина волны в нанометрах, по три знака до и после запятой.
// или перевести в формат через printf

// http://all-ht.ru/inf/prog/c/func/printf.html
/*
f, F - Вывод числа с плавающей точкой в виде [-]dddd.ddd. По умолчанию выводится число с точностью 6,
если число по модулю меньше единицы, то пред десятично точкой выводится ноль, знак указывается только для отрицательных чисел,
с правым выравниванием. Размер по умолчанию sizeof( double ).

 printf ("\n3.2 Вывод числа 123.456 с точностью 2:\n");
 { double d1=123.456;
   printf ("%.2f\n",d1);
 }
Вывод числа 123.456 с точностью 2:
123.46

то есть м.б. достаточно выводить с модификатором "%.3f", без буфера как ниже.
*/
// надо еще знак спереди, трех мало!
// static	char _waveLenght[]={'0','0','0','.','0','0','0'};

// Длина волны лазеров. - нафиг они тут без Рамана
...
//    inputBuff.fill(0);
    ...
    // это еше что? либо new *, или статический массив... но тут у нас не препрцессорная директива, а константа.
//	inputBuff = new char[ARC_INPUT_BUFF_SIZE];
//	for(cInputBuff = 0; cInputBuff < ARC_INPUT_BUFF_SIZE; cInputBuff++)
//	inputBuff[cInputBuff] = NULL; // обнуление буфера.
//    memset(inputBuff, 0, sizeof(inputBuff));
	...
	comPort = 5;//
/* 2021
// 1. Длина волны по умолчанию.
	laserWavelenght=LASER_WAVELENGHT_RED_633;//LASER_WAVELENGHT_GREEN_532;
// 2. Пределы границ рамановского сдвига - считаем от 50 до 3000см-1.
	shiftMin=470;// 50;
	shiftMax=570; // 3000;
*/
...
//// 6. Ширина фронтальной щели в микронах (от 10 до 3000 микрон с шагом 1 мкм).
//	slitWidthFront=1000;
//	slitWidthSide=1000;
...
int ARC_MONOCHROMATOR::Open(int comPort) // подключение спектрометра,
{

	// и еще про С++ стиль открытия порта https://blablacode.ru/programmirovanie/392

	// попробовать заменить символ в строе https://question-it.com/questions/226716/kak-zamenit-odin-simvol-tipa-a-na-chto-to-vrode-10
	// типа string s; s[3] == 'A', с коррекцией как ниже gratingNumber+0x30. но тут максимум 9 портов будет, или надо вводить коррекцию.
	// там же есть про поиск в строке можно так искать байты в ответе счетчика - str.find(findA, pos), так найти оба символа
...
/*
	...
	{					//синхронный режим, без выделения потоков.
		case 1:
		strcpy(Com_Name,"COM1"); break;
		case 2:
		strcpy(Com_Name,"COM2"); break;
//  ...
		default:
		strcpy(Com_Name,"COM3"); break;
	}
*/
// or
...
// http://msdn2.microsoft.com/en-us/library/aa363858.aspx
	...
	hMonochrom = CreateFileA((LPCSTR)Com_Name,GENERIC_READ|GENERIC_WRITE, 0, NULL,OPEN_EXISTING,0, 0);
...
//MessageBox(NULL,"SetupComm OK!","Error",MB_OK);
 ...
 //    std::fill_n(inputBuff, sizeof(inputBuff), 0);

//	Проверка подключения - есть или нет.
//	Write(ARC_CR, sizeof(ARC_CR)); // CR - "возврат каретки", 0x0D.
...
// It is good practice to send a few carriage returns to the SR400 when your program begins
    ...
    Write(ARC_CR); // WriteFile(hMonochrom, ARC_CR, sizeof(ARC_CR), &bc, NULL);
    Write(ARC_CR);
...
*/
 ...
 /*   char* outputBuff = new char[ARC_INPUT_BUFF_SIZE];

    strncpy(outputBuff, arcCommand.c_str(), ARC_INPUT_BUFF_SIZE);
 //   memcpy(outputBuff, arcCommand, arcCommand.size());
// or strcpy(outputBuff, arcCommand.c_str());
//    strncpy(outputBuff, arcCommand.c_str(), sizeof(outputBuff));

    outputBuff[arcCommand.size()] = 0x0D;



    // а вот тут уже не вывести массив, надо было обычный объявлять.. или?
    WriteFile(hMonochrom, outputBuff, arcCommand.size(), &bc, NULL);
...
// может тут и вырезать символы пробела в начале ответа? Можно и вообще все, но если потом спрашивать про решетки - то пробелы потребуются.
	...
	for(int cInputBuff = 0; cInputBuff < arcData.size(); cInputBuff++)
	{
		DWORD bc;
		tmpBuff = 0x00;
		do
...
//	Перевод "плавающего" значение в строчный вид и запись в порт.
// double умножить на 1000, перевести в строку, и вставить десятичный знак.
// 2021 или выводить через модификатор %.3f ?

/*	char tempWaveString[10];
	memset(tempWaveString, 0x20, sizeof(tempWaveString)); // 0x20 means SPACE
    gcvt(targetWavelength, 6, tempWaveString);
    // заменить ноль на пробел
    for(auto x : tempWaveString)
    {
        if(tempWaveString[x] == 0x00)
        tempWaveString[x] = 0x20;
    }

*/

/*
	long tempWaveLength = (long)(targetWavelength * 100000.0);
	char tempWaveString[10];
	ltoa(tempWaveLength, tempWaveString, 10);
	_waveLength[0] = tempWaveString[0];
	_waveLength[1] = tempWaveString[1];
	_waveLength[2] = tempWaveString[2];
	_waveLength[3] = '.';
	_waveLength[4] = tempWaveString[3];
	_waveLength[5] = tempWaveString[4];
	_waveLength[6] = tempWaveString[5];
*/
// функция  *gcvt работает нестабильно. возможно как раз потому что переходит к научной нотации? хз.
/*
#include <stdlib.h> // stdioюр тоже работает...
char *gcvt(double x, int ndigit, char *buf);
The gcvt() function converts x to a NULL-terminated string (similar to the %g format of fprintf(), printf(), sprintf()
— Format and write data) in the array pointed to by buf and returns buf.
It produces ndigit significant digits (limited to an unspecified value determined by the precision of a double)
in %f if possible, or %e (scientific notation) otherwise


#include <stdio.h>
char buffer[16];
int precision = 6;
gcvt(targetWavelength, 6, buffer);
...
void ARC_MONOCHROMATOR::SetGrate(unsigned int grating)
{
