...
static	char SR_HALT[]={'C','H',0x0D}; // "CH\r"  //
static	char SR_RESET[]={'C','R',0x0D};//"CR\r"; 	//
static	char SR_QA[]={'Q','A',0x0D};//"QA\r"; // считывание последнего значения счетчика.	//

// Первоначальный режим работы счетчика.
// Setting the counter mode CM also performs a counter reset
static	char SR_AB_AT_T[]={'C','M','0',0x0D}; // "счет А, В по Т" "CM0\r"	//srABatT
static	char SR_T_AT_10M[]={'C','I','2',',','0',0x0D};	//	Т-вход 10 МГц: "CI2,0\r"	//srTat10M
...
static	char SR_FALL_FRONT_A[]={'D','S','0',',','1',0x0D}; // работа по спадающему фронту

// 2. Время выборки.
static	char SR_TIME_SET[]={'C','P','2',','}; // CP2,XX.XXX задание времени счета //
//	static	char SR_TIME_GET[]={'C','P','2',0x0D}; // считывание времени счета.//

// 4. Команды для установки уровня дискриминации.
// надо подумать над этой командой...
static	char SR_DL_A_SET[]={'D','L','0',','}; // 'DL0,' //
...
static	char SR_SW0[]={'S','W','0',0x0D};// сброс ожидания передачи символа. //
static	char SR_QA1[]={'Q','A','1',0x0D};//"QA1\r"; // считывание первого значения счетчика.	//
// 1. Количество циклов сканирования.
static	char SR_NP_SET[]={'N','P'}; // NP2000\r, значение по-умолчанию. //,'2','0','0','0',0x0D //
static	char SR_DL_A_GET[]={'D','L','0',0x0D}; // DL0. - вход А.	//
...
//	inputBuff = new char[INPUT_BUFF_SIZE];
	...
	comPort = 1; // Счетчик по-умолчанию на СОМ1.
...
void SR_COUNTER::Read(std::array<char, SIZE>& counterData)
{
	...
	char tmpBuff; // = 0x00;
	for(int cInputBuff = 0; cInputBuff < counterData.size(); cInputBuff++)
	{
		DWORD bc;
		tmpBuff = 0x00;
		do
...
unsigned int SR_COUNTER::GetCount() // значение последнего отсчета с А
{
...
unsigned int SR_COUNTER::GetSampleTime()
{
	return	sampleTime;
}

int SR_COUNTER::SetSampleTime(unsigned int setSampleTime) // ms
{
...
signed int SR_COUNTER::GetDiscLevel()
{
	return(discLevel);
}
...
void SR_COUNTER::SetPortNumber(unsigned int numberPort) // Установить номер СОМ-порта.
{
...
unsigned int SR_COUNTER::GetPortNumber()
{
	return(comPort);
}

unsigned int SR_COUNTER::SetDeadTime(unsigned int dTime)
{
...
unsigned int SR_COUNTER::GetDeadTime()
{
	return deadTime;
}
